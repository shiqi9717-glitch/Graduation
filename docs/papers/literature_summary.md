...
为便于承接当前 `interference detector / re-check` 主线，后续建议优先按下面四条线来组织相关工作。

### 6.1 measurement / label design
...
这条线最直接对应你当前项目里的标签定义、评测口径、中文客观题设定，以及“究竟在测什么”这一层的问题。

### 6.2 process monitoring / proxy signals
...
这条线最适合支撑你把当前单次 final-answer 观测，升级成 reasoning trace / anchor / answer drift / re-ask recovery 等过程代理信号分析。

### 6.3 audit + recheck / oversight
...
这条线最接近当前项目里 `detector -> trigger -> re-check` 的 guard 逻辑，更适合把 detector 解释为风险排序和复查预算分配器。

### 6.4 causal / mechanistic mitigation
...
这条线适合在 measurement 和 process proxy 做扎实后，再继续推进更强的机制解释与 mitigation 设计。
