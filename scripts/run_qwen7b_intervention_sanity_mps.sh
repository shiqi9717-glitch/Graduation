...
./.venv/bin/python scripts/build_local_probe_intervention_subset.py \
  ...
  --output-file outputs/experiments/local_probe_qwen7b_intervention_sanity_inputs/qwen7b_intervention_sanity_sample_set.json \
  --strict-positive-limit 50 \
  --high-pressure-limit 50 \
  --control-limit 8

echo "Starting 7B sanity sweep: 108 samples x 4 settings = 432 patched evaluations"

MPLCONFIGDIR=/Users/shiqi/code/graduation-project/.mplconfig \
./.venv/bin/python scripts/run_local_probe_intervention.py \
  --model-name Qwen/Qwen2.5-7B-Instruct \
  --sample-file outputs/experiments/local_probe_qwen7b_intervention_sanity_inputs/qwen7b_intervention_sanity_sample_set.json \
  ...
  --output-dir outputs/experiments/local_probe_qwen7b_intervention_sanity \
  --device mps \
  --dtype float32 \
  ...
  --layer-configs '24-26=24,25,26;24-27=24,25,26,27' \
  --interpolation-scales 0.45,0.6 \
  --flush-every 10 \
  --log-level INFO
