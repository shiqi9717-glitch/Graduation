...
echo "Starting Mistral-7B belief causal transfer: philpapers -> nlp_survey, total n=48, English bridge prompts"
echo "Log file: outputs/logs/mistral7b_belief_causal_transfer_mps.log"
...
./.venv/bin/python scripts/run_belief_causal_transfer.py \
  ...
  --max-length 1024 \
  --flush-every 12 \
  --log-level INFO 2>&1 | tee -a outputs/logs/mistral7b_belief_causal_transfer_mps.log
