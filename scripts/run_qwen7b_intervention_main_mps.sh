...
./.venv/bin/python scripts/build_local_probe_intervention_subset.py \
  ...
  --output-file outputs/experiments/local_probe_qwen7b_intervention_main_inputs/qwen7b_intervention_main_sample_set.json \
  --strict-positive-limit 50 \
  --high-pressure-limit 50 \
  --control-limit 8

echo "Starting 7B main intervention runs on 108 samples"
...
./.venv/bin/python scripts/run_local_probe_intervention.py \
  ...
  --output-dir outputs/experiments/local_probe_qwen7b_intervention_main/baseline_24_26_s06 \
  --device mps \
  --dtype float32 \
...
./.venv/bin/python scripts/run_local_probe_intervention.py \
  ...
  --sample-file outputs/experiments/local_probe_qwen7b_intervention_main_inputs/qwen7b_intervention_main_sample_set.json \
  --reference-mechanistic-run-dir outputs/experiments/local_probe_qwen7b_mechanistic_mps_fp32_generalization/Qwen_Qwen2.5-7B-Instruct/20260422_134717 \
  --output-dir outputs/experiments/local_probe_qwen7b_intervention_main/aggressive_24_27_s06 \
  --device mps \
  --dtype float32 \
  --methods baseline_state_interpolation \
  --sample-types strict_positive,high_pressure_wrong_option,control \
  --direction-sample-types strict_positive,high_pressure_wrong_option \
  --layer-configs '24-27=24,25,26,27' \
  --interpolation-scales 0.6 \
  --flush-every 10 \
  --log-level INFO
...
./.venv/bin/python scripts/run_local_probe_intervention.py \
  ...
  --sample-file outputs/experiments/local_probe_qwen7b_intervention_main_inputs/qwen7b_intervention_main_sample_set.json \
  --reference-mechanistic-run-dir outputs/experiments/local_probe_qwen7b_mechanistic_mps_fp32_generalization/Qwen_Qwen2.5-7B-Instruct/20260422_134717 \
  --output-dir outputs/experiments/local_probe_qwen7b_intervention_main/subtraction_control_24_26_s06 \
  --device mps \
  --dtype float32 \
  --methods late_layer_residual_subtraction \
  --sample-types strict_positive,high_pressure_wrong_option,control \
  --direction-sample-types strict_positive,high_pressure_wrong_option \
  --layer-configs '24-26=24,25,26' \
  --subtraction-scales 0.6 \
  --flush-every 10 \
  --log-level INFO
