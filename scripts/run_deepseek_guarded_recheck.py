...
def _build_sample_manifest(
    ...
    df = scored_df.copy()
    df["strict_label"] = pd.to_numeric(df.get("strict_label", 0), errors="coerce").fillna(0).astype(int)
    df["is_hard_negative"] = pd.to_numeric(df.get("is_hard_negative", 0), errors="coerce").fillna(0).astype(int)
    df["explicit_wrong_option"] = pd.to_numeric(df.get("explicit_wrong_option", 0), errors="coerce").fillna(0).astype(int)
    df["is_control"] = pd.to_numeric(df.get("is_control", 0), errors="coerce").fillna(0).astype(int)
    df["interference_score"] = pd.to_numeric(df.get("interference_score", 0.0), errors="coerce").fillna(0.0)
    df["triggered"] = (df["interference_score"] >= float(threshold)).astype(int)
    df["condition_id"] = df.get("arm_id", "")
    ...
    manifest["sample_group"] = manifest["stratum"] if "stratum" in manifest.columns else ""
    return manifest
...
def _merge_results(manifest: pd.DataFrame, judge_results: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    def _ensure_text_column(df: pd.DataFrame, column: str) -> pd.Series:
    ...
    out["raw_answer"] = _ensure_text_column(out, "predicted_answer").str.upper().str.strip()
    out["recheck_answer"] = _ensure_text_column(out, "recheck_answer").str.upper().str.strip()
    ...
    out["ground_truth"] = _ensure_text_column(out, "ground_truth").str.upper().str.strip()
    out["wrong_option"] = _ensure_text_column(out, "wrong_option").str.upper().str.strip()
    ...
    out["judge_answer"] = _ensure_text_column(out, "recheck_answer")
    out["judge_raw_response_text"] = _ensure_text_column(out, "recheck_raw_response_text")
    out["judge_success"] = _ensure_numeric_column(out, "recheck_success", 0).astype(int)
    out["judge_latency_ms"] = _ensure_numeric_column(out, "recheck_latency_ms", 0.0)
    out["judge_error_message"] = _ensure_text_column(out, "recheck_error_message")
    out["judge_model_name"] = _ensure_text_column(out, "recheck_model_name")
    out["judge_changed_answer"] = _ensure_numeric_column(out, "recheck_changed_answer", 0).astype(int)
    return out
...
def _group_metrics(df: pd.DataFrame, group_name: str, value: str) -> Dict[str, Any]:
    ...
    if group_name == "strict_positive":
        sdf = sdf[sdf["strict_label"].fillna(0).astype(int) == 1]
    elif group_name == "hard_negative":
        sdf = sdf[sdf["is_hard_negative"].fillna(0).astype(int) == 1]
    elif group_name == "explicit_wrong_option":
        sdf = sdf[pd.to_numeric(sdf["explicit_wrong_option"], errors="coerce").fillna(0).astype(int) == 1]
    elif group_name == "model_name":
        sdf = sdf[sdf["model_name"] == value]
    elif group_name == "condition_id":
        sdf = sdf[sdf["arm_id"] == value]
    if sdf.empty:
        return {
...
def _build_summary(
    ...
    summary = {
        ...
        "recheck_models_used": sorted(final_df["recheck_model_name"].fillna("").astype(str).unique().tolist()),
        ...
        "recheck_skipped_samples": int(
            _ensure_text_series(final_df, "recheck_skip_reason").ne("").sum()
...
def _build_group_rows(final_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for model_name in sorted(final_df["model_name"].dropna().astype(str).unique().tolist()):
        rows.append(_group_metrics(final_df, "model_name", model_name))
    top_conditions = final_df["arm_id"].fillna("").astype(str).value_counts().head(12).index.tolist()
...
async def _async_main(args: argparse.Namespace) -> int:
    scored_df = _read_dataset(Path(args.scored_dataset))
