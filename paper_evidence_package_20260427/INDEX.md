...
| Appendix / Qwen clean | Qwen 3B clean protocol provides appendix-level secondary positive support under the clean protocol | `results_transfer/qwen3b_clean_positive_confirmation_run/belief_causal_summary.csv` | Appendix clean-protocol note | ready | less-oracle mechanistic probe only; drift/compliance 明显下降，recovery 小幅提升，negative control 未复现收益 |
...
| Appendix / Metric audit | Metric numerator/denominator, bootstrap/CI, and held-out leakage audit | `appendix_materials/metric_audit_note_20260501.md` | Appendix audit note | ready | 95% CIs are part of the frozen result package; exact bootstrap repetition count and CI variant are not independently confirmed in the current audit |
| Appendix / Qwen clean note | Clean-protocol Qwen 3B/7B/14B belief causal transfer boundary note | `appendix_materials/qwen_clean_protocol_appendix_note.md` | Appendix note / evidence boundary note | ready | 统一收束 3B appendix-level secondary positive support、7B weak / boundary、14B n=48 cautionary |
