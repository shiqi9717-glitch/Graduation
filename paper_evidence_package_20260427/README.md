...
- Qwen clean protocol 新增结果中，3B 只作为 appendix-level secondary positive support under the clean protocol；7B 只作为 weak / boundary evidence；14B `n=48` 只作为 cautionary / boundary evidence。
...
- `results_transfer/`
  - Qwen 14B secondary confirmation、Qwen 3B clean appendix-level secondary support、Qwen 7B clean boundary evidence、Qwen 14B `n=48` cautionary evidence、GLM tradeoff、Llama bridge、Mistral exploratory。
...
- Qwen clean protocol 中：3B 只作为 appendix-level secondary positive support under the clean protocol；7B 只作为 weak / boundary evidence；14B `n=48` 只作为 cautionary / boundary evidence。
...
`appendix_materials/metric_audit_note_20260501.md` 则用于归档 metric numerator/denominator、bootstrap / CI、以及 held-out leakage 的最新审计边界。它的作用是收紧表述，不是独立确认 frozen closure bootstrap repetition count 或 exact CI variant。
